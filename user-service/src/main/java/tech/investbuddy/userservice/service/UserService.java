...
public class UserService {
    ...
    public UUID createUser(UserRequest userRequest) {
        ...
        // Envoyez l'email de vérification via Mailgun
        /*String verificationLink = "http://localhost:8080/api/v1/user/auth/verify-email?token=" + verificationToken;
        ...
        String message = String.format("{\"email\":\"%s\",\"verificationToken\":\"%s\"}",
                user.getEmail(),
                verificationToken);
        kafkaProducer.sendMessage("user-created", message);
        return user.getId();
    }
...
//    public void updateUserAfterEmailValidation(UUID userId){
//
//    }
